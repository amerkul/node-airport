...
class FlightService {
    ...
    async retrieveAllWays(filter: FlightFilter, offset: number, size: number): Promise<Flight[]> {
        try {
    ...
    async update(newData: Flight, id: number): Promise<Flight> {
        try {
            newData.id = id;
    ...
    async retrieveAll(offset: number, size: number): Promise<Flight[]> {
        try {
            return await flightRepository.search(new FlightFilter(), offset, size);
        } catch(err: any) {
            throw new CustomError(500, err.message);
        }
    }
